...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% $Author: Lumbrer $    $Date: 2016/12/16 $    $Revision: 0.7$
...
%
% C) Chapter C covers the use of reduced scores from both Principal
...
if PCAswitch % {CHAPTER A}
    ...
    [Princ_comp_90_w,Features_Norm_PCA_w]=pca(Features_w_Norm,...
    ...
    FiguresFeat.f3=findobj('type','figure','Name','PCA Pairwise Biplot - White Wine');
    if ishghandle(FiguresFeat.f3)
        close(FiguresFeat.f3)
    end
    FiguresFeat.f3=figure('Name','PCA Pairwise Biplot - White Wine');
    ...
    % A biplot can help us assess the quality of the component loadings or
    % contributions from each feature to the principal components. In order
    % to understand the loadings Thurstone brought forward five ideal 
    ...
    FiguresFeat.f4=findobj('type','figure','Name','PCA Biplot - White Wine');
    if ishghandle(FiguresFeat.f4)
        close(FiguresFeat.f4)
    end
    FiguresFeat.f4=figure('Name','PCA Biplot - White Wine');
    ...
    FiguresFeat.f5=findobj('type','figure','Name','PCA Image Display - White Wine');
    if ishghandle(FiguresFeat.f5)
        close(FiguresFeat.f5)
    end
    FiguresFeat.f5=figure('Name','PCA Image Display - White Wine');
    % Notice how we need to use absolute value to produce the plot as the
    % colour scale requires positive values.  
    imagesc(abs(Princ_comp_90_w(:,1:3)))
    colorbar
    colormap parula
    ax=gca;
    ...
    subplot(2,1,[1:2])
    glyphplot(abs(Princ_comp_90_w'),'standardize','off','VarLabels',...
